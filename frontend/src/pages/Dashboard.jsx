  ...
  const filteredTasks = useMemo(() => {
      ...
      return tasks
        .filter((t) => {
          ...
          const doc = (t.cliente_documento || "").toLowerCase();
        ...
        .filter((t) =>
          filtroStatus ? (t.status || "").toLowerCase() === filtroStatus : true
        );
    ...
    <div className="container">
      ...
      <div className="table-filters-container">
        <div className="search-wrapper">
          ...
          <input
            type="text"
            placeholder="Buscar por nome ou CNPJ/CPF do cliente..."
        ...
        <tbody>
          {currentTasks.length > 0 ? (
            currentTasks.map((t) => {
              ...
              const isRetry = status === "retry_dps"; // Detecta se é o retry
              const isSelected = selectedTaskIds.includes(t._id);

              // Se for retry, forçamos a cor de 'pending' (amarelo/azul) para não quebrar o CSS
              const statusClass = isRetry ? "pending" : status;
              ...
              return (
                <tr key={t._id} className={isSelected ? "row-selected" : ""}>
                  ...
                  {/* 2. CNPJ/CPF */}
                  <td>{formatDocumento(t.cliente_documento)}</td>
                  ...
                  <td>
                    ...
                    >
                      {/* Aqui ele vai buscar o label "Emitindo..." se você atualizou o objeto statusLabels */}
                  ...
                  <td style={{ textAlign: "center", position: "relative" }}>
                    {isAccepted ? (
                      // Lógica para notas aceitas (Menu de 3 pontos)
                      <>
                        <button
                          className="icon-btn"
                          onClick={(e) => {
                        ...
                        {openMenuId === t._id && (
                          <div className="action-menu" ref={menuRef}>
                            ...
                            {t.has_pdf ? (
                              <button
                                className="action-item"
                                onClick={() => handleDownloadGuia(t._id)}
                              >
                                <span className="pdf-chip">PDF</span> Download DANFS-e
                              </button>
                            ) : (
                              <div
                                className="action-item disabled-tip"
                                title="A prefeitura está processando o documento. O PDF estará disponível em até 2 minutos."
                                style={{ display: "flex", alignItems: "center", gap: "6px", color: "#888", cursor: "help" }}
                              >
                                <Loader2 className="animate-spin" size={16} color="#e0c51a" />
                                Gerando PDF...
                              </div>
                            )}
                      ...
                      // Lógica para notas NÃO aceitas
                      <>
                        {isRetry ? (
                          // Se for retry, mostra o Loader girando
                          <div
                            style={{ display: "flex", justifyContent: "center" }}
                            title="O sistema está reprocessando esta nota automaticamente"
                          >
                            <Loader2
                          ...
                          // Se for cancelada ou erro comum, mostra N/A ou ?
