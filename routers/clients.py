...
def create_client(client: ClientCreate, current_user: UserInDB = Depends(get_current_user)):
    raw_payload = client.dict(exclude_unset=True)
    ...
    if cep and len(cep) == 8:
        try:
            ...
            # ? SÓ cria IBGE se o front NÃO enviou o campo
            if "codigoIbge" not in raw_payload:
                data["codigoIbge"] = sanitize_document(str(resp.get("ibge") or ""))
...
def update_client(client_id: str, client: ClientUpdate, current_user: UserInDB = Depends(get_current_user)):
    ...
    for k, v in data.items():
        if isinstance(v, str):
            if v.strip() == "":
                # permite limpar explicitamente o código IBGE
                if k == "codigoIbge":
                    clean_data[k] = None
            else:
                clean_data[k] = v
    ...
    if "codigoIbge" in data:
        if data["codigoIbge"] in (None, "", " "):
            # remove o campo do Mongo
            unset_fields["codigoIbge"] = ""
            data.pop("codigoIbge")
        else:
            data["codigoIbge"] = sanitize_document(str(data["codigoIbge"]))
    ...
    result = db.clients.update_one(query, update_doc)
